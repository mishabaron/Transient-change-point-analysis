...
threshold = function(n,s,alpha,NrunsH){
  ...
  for (run in 1:NrunsH){
   	 ...
   	 z = log( dnorm(X,0,s)/dnorm(X,0,1) );	# log-likelihood ratios
...
MLE.single = function(X,s,h){  # Estimation of a single transient change-point
  # X = data sequence, mu is the post-change mean. N(0,1) -> N(0,s)
  n = length(X); s1 = s; m0 = 0; mu1 = 0; s0 = 1; 
